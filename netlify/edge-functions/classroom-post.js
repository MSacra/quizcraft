// Posts a grade/comment to Google Classroom
export default async function handler(request) {
  ...
  let tokenData;
  try { tokenData = JSON.parse(atob(match[1])); } catch(e) { return new Response(JSON.stringify({ error: 'Invalid token.' }), { status: 401, headers }); }
  ...
  try {
    ...
    const submission = subsData.studentSubmissions?.[0];
    if (!submission) return new Response(JSON.stringify({ error: 'Student submission not found in Classroom.' }), { status: 404, headers });

    // Post private comment
    await fetch(
      `https://classroom.googleapis.com/v1/courses/${courseId}/courseWork/${courseWorkId}/studentSubmissions/${submission.id}:modifyAttachments`,
      {
        method: 'POST',
